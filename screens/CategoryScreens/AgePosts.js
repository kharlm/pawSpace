...
import { getPosts, likePost, unlikePost, getAdopt,getAgePosts,flagPost } from '../../actions/post'
...
class AgePosts extends React.Component {

  
    async componentDidMount() {
        const firstAge = this.props.navigation.getParam('firstAge','');
        const secondAge = this.props.navigation.getParam('secondAge','');
        this.props.getAgePosts(firstAge,secondAge)
        this.props.getPosts()
      ...
      likePost = (post,firstAge,secondAge) => {
      ...
      render(){
        ...
        if(typeof this.props.post.breedFeed != 'undefined' && this.props.post.breedFeed.length<1){
          ...
          >
            There are no {firstAge} to {secondAge} year old dogs
        </Text>
            )
        }
        else{
         ...
         <View style={{ flex: 1}}>
          ...
          >
             {firstAge} to {secondAge} year old Dogs
        </Text>
          ...
          renderItem={({ item }) => {
            ...
            return (   
              <View>
                ...
                <DoubleClick
                      doubleTap={() => {this.likePost(item,firstAge,secondAge)}}
                      delay={200}>
                     {
                        item.isVideo===true ?
                        <ViewportAwareVideo
                ...
                <TouchableOpacity onPress={() => this.likePost(item,firstAge,secondAge)} >
    ...
    const mapDispatchToProps = (dispatch) => {
        return bindActionCreators({ getPosts, likePost, unlikePost, getUser, getAdopt,getDog,getAgePosts,flagPost}, dispatch)
      }
      ...
      export default connect(mapStateToProps, mapDispatchToProps)(AgePosts)
